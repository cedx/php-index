...
// package.json
var package_default = {
  bugs: "https://github.com/cedx/php-index/issues",
  description: "A PHP directory index generator.",
  homepage: "https://github.com/cedx/php-index",
  license: "MIT",
  name: "@cedx/php-index",
  repository: "cedx/php-index",
  type: "module",
  version: "2.1.0",
  author: {
  ...
  bin: {
    php_index: "./bin/php_index.cjs"
  },
  devDependencies: {
    "@cedx/php-minifier": "^2.0.0",
    "@fontsource-variable/material-symbols-rounded": "^5.0.24",
    "@lit/localize": "^0.12.1",
  ...
  scripts: {
    prepack: "gulp",
    start: "gulp cli && node bin/php_index.cjs --help"
  }
};
...
`;
async function main() {
  const { positionals, values } = (0, import_node_util2.parseArgs)({ allowPositionals: true, options: {
  ...
  if (!positionals.length)
    throw Error("You must provide the path of the output directory.");
  const root = (0, import_node_path3.join)(__dirname, "..");
  const input = await (0, import_promises2.mkdtemp)((0, import_node_path3.join)((0, import_node_os4.tmpdir)(), "phpindex-"));
  for (const folder of ["lib", "www"])
    await (0, import_promises2.cp)((0, import_node_path3.join)(root, folder), (0, import_node_path3.join)(input, folder), { recursive: true });
  await (0, import_promises2.rm)((0, import_node_path3.join)(input, "www/index.php"));
  const replaceInFile = async (file, search, replace) => (0, import_promises2.writeFile)(file, (await (0, import_promises2.readFile)(file, { encoding: "utf8" })).replace(search, replace));
  ...
  const output = (0, import_node_path3.resolve)(positionals[0]);
  await (0, import_promises2.mkdir)(output, { recursive: true });
  return execa("php", [(0, import_node_path3.join)(root, "bin/php_index.php"), "--input", input, "--output", output].concat(values.compress ? ["--compress"] : []));
}
main().catch((error) => {
